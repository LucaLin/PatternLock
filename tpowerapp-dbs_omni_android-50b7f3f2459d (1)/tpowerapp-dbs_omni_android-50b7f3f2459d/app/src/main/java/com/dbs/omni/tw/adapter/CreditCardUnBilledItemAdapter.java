...
public class CreditCardUnBilledItemAdapter extends BaseAdapter {
    private Context context;
    private List<ConsumptionItem> list;
    private LayoutInflater inflater;

    public CreditCardUnBilledItemAdapter(Context context) {
        this.context = context;
        inflater = LayoutInflater.from(context);
    }

    public CreditCardUnBilledItemAdapter(List<ConsumptionItem> list, Context context) {
        this.context = context;
        this.list = list;
        inflater = LayoutInflater.from(context);
    }
        ...
        switch (item.getItemType()) {
            case TITLE:
                ViewTitleHolder viewTitleHolder;
                    //取得listItem容器 view
                    convertView = inflater.inflate(R.layout.consumption_header_item, null);

                    //建構listItem內容view
                    viewTitleHolder = new ViewTitleHolder(convertView);

                    convertView.setTag(viewTitleHolder);
                    setDataViewValue(viewTitleHolder, item);
                break;
